...
async function searchShows(query) {
  ...
  let searchObjects = (await axios.get('http://api.tvmaze.com/search/shows', {
    params: {
      q: query
    }
  })).data
  ...
  for (let searchObject of searchObjects) {
    let image = searchObject.show.image ? searchObject.show.image.medium : 
    "https://store-images.s-microsoft.com/image/apps.65316.13510798887490672.6e1" +
    "ebb25-96c8-4504-b714-1f7cbca3c5ad.f9514a23-1eb8-4916-a18e-99b1a9817d15?mode=scale&q=90&h=300&w=300";
...
function populateShows(shows) {
    ...
    let $item = $(
      `<div class="col-md-6 col-lg-3 Show" data-show-id="${show.id}">
         <div class="card" data-show-id="${show.id}" data-show-name="${show.name}">
           ...
           <button type="button" class="show-episodes btn btn-primary" 
  ...
  $("#shows-list").on("click", ".show-episodes", async function (e) {
    ...
    let title = $(e.target).parent().data().showName;
    $('#exampleModalLongTitle').text(`Episodes of ${title}`);

    // Display episodes of targeted show
...
// Function that takes in list of episodes and displays them to user
